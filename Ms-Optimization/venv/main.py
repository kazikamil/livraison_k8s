    ...
    for i, cluster in enumerate(clusters):
        ...
        distances = np.array([
            (calculate_time(n_depart, np.array(cmd["depart"])) +
             ...
             calculate_time(n_depart, np.array(cmd["arrivee"])) +
...
async def add_order(request:Request):
    ...
    '''
    print(1)
    url_cmd="http://ms-commande:5000/commandes"
    url_livreur="http://ms-suivi:5000/api/livreursLocations"
    ...
    if response.status_code == 200:
     ...
     # generate_map(clusters)
      ...
      post_url = "http://ms-suivi:5000/livreur/route"  # Replace "url.com" with your actual URL
      post_data = {
        "trajet": trajet,
...
async def accept(request:Request):
     ...
     url_cmd="http://ms-commande:5000/service-commande/commandes"
     response = requests.get(url_cmd)
     print(url_cmd)
     if response.status_code == 200:
       all_commandes = response.json()
...
async def refuse(request:Request):
     ...
     print(livreur)
     commande=data['commande']
     url_cmd=f"http://ms-commande:5000/commandes/{commande}"
